...
class ConditioningMLP(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.SiLU(),  # Using SiLU activation for smooth non-linearity
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if self.spatial_dims == 1:
            x = x.reshape(batch, height, inner_dim).permute(0, 2, 1).contiguous()   
        if self.spatial_dims == 2:
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.spatial_dims == 1:
            x = x.view(batch, channel, height).transpose(1, 2)
        if self.spatial_dims == 2:
...
class DiffusionModelUNet(nn.Module):
    ...
    def __init__(
        ...
        attention_levels: Sequence[bool] = (False, False, False, False),
        norm_num_groups: int = 32,
        norm_eps: float = 1e-6,
        resblock_updown: bool = False,
        ...
        if with_conditioning:
            ...
            self.conditioning_mlp = ConditioningMLP(input_dim=256, hidden_dim=256, output_dim=512)   
    ...
    def forward(
        ...
        context = self.conditioning_mlp(context)
